...
Parallel_function <- function(i, Train, e){
  ...
  cutoff        = seq(0.98,0.95,length.out = 50)# probability cutoff for predicting fraud
...
for (e in start:end){
  ...
  write.csv(Tunning_out , paste0(path, Model , "_",Sampling,"_Tunning_out_no_add_feature_10f.csv") , row.names = F)
  ...
  write.csv(summary_table , paste0(path, Model , "_", Sampling, "_Tunning_summary_out_no_add_feature_10f.csv") , row.names = F)
