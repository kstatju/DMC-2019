...
Parallel_function <- function(i, Train, e){
  ...
  F_smote       = T                       # apply SMOTE so solve class imbalance
  n_folds       = 5                       # number of folds for cross validation
  n_trial       = 20                      # number of trials for repeated CV
  cutoff        = seq(0.98, 0.93, by = -0.001)# probability cutoff for predicting fraud
  ...
  Config_Logistic   = function(){
    Exp_Params = list()
    Exp_Params[[1]] = c(.95) #seq(0.85, 1, length.out = 100)                       #alpha
  ...
  Fit_Logistic   = function(df , df_folds = NULL , n_folds = 5, params = NULL){
    ...
    Logistic_models = lapply(c(1:n_folds) , function(x){
      ...
      if(Sampling == "none"){
    	...
    	nm = names(df_train)
    	xnm = nm[!(nm%in%c('fraud'))]
    	# aa = model.matrix(fraud ~.,data = df_train)
...
n_folds       = 5                       # number of folds for cross validation
n_trial       = 15                      # number of trials for repeated CV
