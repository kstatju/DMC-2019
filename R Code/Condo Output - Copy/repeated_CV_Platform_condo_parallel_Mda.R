...
Parallel_function <- function(i, Train, e){
  ...
  cutoff        = seq(0.999,0.94,by = -0.005)# probability cutoff for predicting fraud
  ...
  Config_Mda   = function(){
    Exp_Params = list()
    Exp_Params[[1]] = c(seq(1,15,by = 1))                       #subclass0
    Exp_Params[[2]] = c(seq(1,2,3,by = 1))                       #subclass1
    Exp_Params[[3]] = c(seq(1, 300, 4))                       #lambda
