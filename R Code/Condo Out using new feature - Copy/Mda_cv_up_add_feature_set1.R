...
Parallel_function <- function(i, Train, e){
  ...
  Config_Mda   = function(){
    ...
    Exp_Params[[3]] = c(seq(1, 300, by = 5))                       #lambda
...
for (e in start:end){
  ...
  write.csv(Tunning_out , paste0(path, Model , "_",Sampling,"_Tunning_out_set1.csv") , row.names = F)
  ...
  write.csv(summary_table , paste0(path, Model , "_", Sampling, "_Tunning_summary_out_set1.csv") , row.names = F)
