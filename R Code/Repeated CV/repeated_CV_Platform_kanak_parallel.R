...
Parallel_function <- function(i, Train, e){
  ...
  for (k in 1:ncombin){
    ...
    for(p in 1:length(cutoff)){
      ...
      folds_out = data.frame("Exp_ID" = rep(e, n_folds),
                             "Cutoff" = rep(cutoff[p], n_folds),
                             "Trial"  = rep(i, n_folds),
...
path = "file:///D:/ISU/Data Mining Cup DMC/DMC 2019/R code/feature_create/Final_data_with_feature/"
