train = read.csv(file = "D:/ISU/Data Mining Cup DMC/DMC 2019/Data/train.csv", header = T, sep = "|")
test = read.csv(file = "D:/ISU/Data Mining Cup DMC/DMC 2019/Data/test.csv", header = T, sep = "|")
test$fraud = NA
train$dttype = "Train"
test$dttype = "Test"
...
ggplot(combin_dt %>%
  ...
  geom_histogram(aes(weight = Prop), stat = 'count', position = 'dodge')+
  theme(legend.text=element_text(size=8))+theme(axis.title=element_text(size=8))
