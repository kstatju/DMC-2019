...
Config_svm   = function(){
  Exp_Params = list()
  Exp_Params[[1]] = c("polynomial") #kernel
...
for (e in start:end){
  ...
  write.csv(Tunning_out , paste0(path, Model , "_" , Sampling, "_" , "Tunning_out_with_new_dt_polynomial.csv") , row.names = F)
...
perf_in = read.csv(paste(path, "svm_up_Tunning_out_with_new_dt_polynomial.csv", sep = ""))
...
write.csv(perf_summary , paste(path, "svm_up_summary_with_new_dt_polynomial.csv",sep = ""))
