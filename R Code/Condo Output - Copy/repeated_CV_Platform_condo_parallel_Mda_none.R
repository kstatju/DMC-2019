...
Parallel_function <- function(i, Train, e){
  ...
  cutoff        = seq(0.999,0.84,by = -0.005)# probability cutoff for predicting fraud
  ...
  Config_Mda   = function(){
    Exp_Params = list()
    Exp_Params[[1]] = 3#c(seq(1,15,by = 1))                       #subclass0
    Exp_Params[[2]] = 1#c(seq(1,2,3,by = 1))                       #subclass1
    Exp_Params[[3]] = 200#c(seq(1, 500, 5))                       #lambda
    
    
    # Exp_Params[[1]] = c(seq(1,10,by = 1))                           #subclass0
    # Exp_Params[[2]] = c(seq(1,10,by=1))                             #subclass0
    # Exp_Params[[3]] = c(seq(1, 1000, 10))                           #lambda
    
    
    Experiment = expand.grid(Exp_Params)
    Experiment = data.frame(id = 1:nrow(Experiment), Experiment)
    colnames(Experiment) = c('id', paste0("P",c(1:3)))
    
    
    Exp_cols = c("P1.subclass0"            , 
                 "P2.subclass1"           , 
                 "P3.lambda")
  ...
  #------------------------------------------------------------------------------------
  Set_Mda_Params   = function(Exp = NULL , id = 1){
    
    p = list(subclass0       = Exp$P1[id],
             subclass1       = Exp$P2[id],
             lambda          = Exp$P3[id])
    return(params = p)
  }
  ...
  Set_params      = function (Model, e){
    if(Model == "Mda"){return(Set_Mda_Params(Mda_Exp, e))}
  }
  ...
  Fit_Mda   = function(df , df_folds = NULL , n_folds = 5, params = NULL){
    
    Mda_models = lapply(c(1:n_folds) , function(x){
      if(Sampling == "down"){
      ...
      if(Sampling == "up"){
        df_train = upSample(x=df[df_folds$train_index[[x]],-"fraud"] , 
                            y=as.factor(df[df_folds$train_index[[x]],]$fraud) , 
                            yname = "fraud")
      }
      if(Sampling == "smote"){
      ...
      mda::mda(formula = fraud~., df_train, 
               subclasses = c(params[[1]],params[[2]]), 
               method = gen.ridge, 
               lambda = params[[3]])
    })
    
    yhat_train = lapply(c(1:n_folds), function(x) predict(Mda_models[[x]], newdata = df[df_folds$train_index[[x]],], type = "posterior")[,2])
    yhat_test  = lapply(c(1:n_folds), function(x) predict(Mda_models[[x]], newdata = df[df_folds$test_index[[x]],] , type = "posterior")[,2])
    
    return(list(model      = Mda_models, 
                Params     = params,
                yhat_test  = yhat_test,
  ...
  Train_model = function(Model = NULL , df = NULL , df_folds = NULL , n_folds=5 , params=NULL){
    if(Model == "Mda"){return(Fit_Mda(df , df_folds ,n_folds ,params ))}
  }
  ...
  #------------------------------------------------------------------------------------
  if(Model == "Mda"){
    Mda_Config = Config_Mda()
    Mda_Exp    = Mda_Config$Exp
    Exp_Params  = Mda_Config$params
    tune_names  = Mda_Config$names
    ncombin = nrow(Mda_Exp)
  }
...
for (e in start:end){
  ...
  # fold_estimates = lapply(X = 1:n_trial, FUN = function(i) Parallel_function(i = i, Train = Train, e = e))
  ...
  write.csv(Tunning_out , paste0(path, Model , "_",Sampling,"_Tunning_out.csv") , row.names = F)
  ...
  write.csv(summary_table , paste0(path, Model , "_", Sampling, "_Tunning_summary_out.csv") , row.names = F)
  ...
  summary_table = summary_table %>% group_by(Cutoff               ,
                                             P1.subclass0         , 
                                             P2.subclass1         , 
                                             P3.lambda            ) %>%
    summarise_at(c("Train_Accuracy"         ,
