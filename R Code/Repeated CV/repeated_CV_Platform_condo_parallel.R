...
Parallel_function <- function(i, Train, e){
  ...
  Config_Fda   = function(){
    Exp_Params = list()
    Exp_Params[[1]] = c(2, 3)                       #degree
    Exp_Params[[2]] = c(2, 3)                          #penalty
    Exp_Params[[3]] = c(80)                                  #nk
    Exp_Params[[4]] = c(0.0001)                              #thresh
    Exp_Params[[5]] = c(40)                       #minspan
    Exp_Params[[6]] = c( 50)                       #endspan
    Exp_Params[[7]] = c(0)                                   #fast.k
    Exp_Params[[8]] = c("backward")                          #pmethod
    Exp_Params[[9]] = c( 40)                      #nprune
    Exp_Params[[10]] = c(5)                                  #nfold
    Exp_Params[[11]] = c(1)                                  #ncross
    Exp_Params[[12]] = c( 10)                         #Adjust.endspan
    Exp_Params[[13]] = c(F)                                  #Get.leverages
    
    # Exp_Params[[1]] = c(2, 3, 4, 5, 6)                           #degree
    # Exp_Params[[2]] = c(2, 3, 4, 5)                      #penalty
    # Exp_Params[[3]] = c(80)                                  #nk
    # Exp_Params[[4]] = c(0.0001)                              #thresh
    # Exp_Params[[5]] = c(5, 10, 20, 40)                        #minspan
    # Exp_Params[[6]] = c(10, 20, 40, 50)                      #endspan
    # Exp_Params[[7]] = c(0)                                   #fast.k
    # Exp_Params[[8]] = c("backward")                          #pmethod
    # Exp_Params[[9]] = c(5, 10, 20, 30, 40)                      #nprune
    # Exp_Params[[10]] = c(5)                                  #nfold
    # Exp_Params[[11]] = c(1)                                  #ncross
    # Exp_Params[[12]] = c(0, 3, 6, 10)                         #Adjust.endspan
    # Exp_Params[[13]] = c(F)                                  #Get.leverages
  ...
  Fit_Fda   = function(df , df_folds = NULL , n_folds = 5, params = NULL){
    ...
    Fda_models = lapply(c(1:n_folds) , function(x){
      mda::fda(formula = fraud~.,
               data = df[df_folds$train_index[[x]],],
               method = earth,
               degree          = params[[1]],
               penalty         = params[[2]],
               nk              = params[[3]],
  ...
  for (k in 1:ncombin){
    ...
    param_out = matrix ( rep( unlist(params) , length(cutoff)) , ncol = length(params) ,byrow = T)
    colnames(param_out) = tune_names
    Exp_out_1 = cbind(Exp_out , param_out)
    Tunning_out = rbind(Tunning_out , Exp_out_1)
