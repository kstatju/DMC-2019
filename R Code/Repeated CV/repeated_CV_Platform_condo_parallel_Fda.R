...
Parallel_function <- function(i, Train, e){
  ...
  n_folds       = 5                       # number of folds for cross validation
  n_trial       = 10                      # number of trials for repeated CV
  cutoff        = c(0.99)#seq(0.999,0.94,by = -0.01)# probability cutoff for predicting fraud
  ...
  Config_Fda   = function(){
    ...
    Exp_Params[[2]] = c(2, 3, 4, 5)                          #penalty
    Exp_Params[[3]] = c(80)                                  #nk
    Exp_Params[[4]] = c(0.0001)                              #thresh
    Exp_Params[[5]] = 5 #c(5, 8, 12)                        #minspan
    Exp_Params[[6]] = 50 #c(50, 60, 70)                       #endspan
    Exp_Params[[7]] = c(0)                                   #fast.k
    Exp_Params[[8]] = c("backward")                          #pmethod
    Exp_Params[[9]] = 18 #c(18, 20, 22)                      #nprune
    Exp_Params[[10]] = c(5)                                  #nfold
    Exp_Params[[11]] = c(1)                                  #ncross
    Exp_Params[[12]] = 7 #c(5, 6, 7, 8)                        #Adjust.endspan Max = 10
  ...
  Fit_Fda   = function(df , df_folds = NULL , n_folds = 5, params = NULL){
    ...
    Fda_models = lapply(c(1:n_folds) , function(x){
      ...
      pp = mean(df[df_folds$train_index[[x]],'fraud'])
    ...
    yhat_train = lapply(c(1:n_folds), function(x) as.numeric(as.character(predict(Fda_models[[x]] , df[df_folds$train_index[[x]],] , type = "posterior")[,2])))
    yhat_test  = lapply(c(1:n_folds), function(x) as.numeric(as.character(predict(Fda_models[[x]] , df[df_folds$test_index[[x]],]  , type = "posterior")[,2])))
...
path = "D:/ISU/Data Mining Cup DMC/DMC 2019/Data/" #"/work/STAT/kanak/"              #
