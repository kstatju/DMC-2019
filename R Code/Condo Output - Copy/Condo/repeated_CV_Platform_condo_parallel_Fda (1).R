...
Parallel_function <- function(i, Train, e){
  ...
  n_folds       = 5                       # number of folds for cross validation
  n_trial       = 10                      # number of trials for repeated CV
  cutoff        = seq(0.99,0.91,by = -0.01)# probability cutoff for predicting fraud
  ...
  Config_Logistic   = function(){
    Exp_Params = list()
    Exp_Params[[1]] = c(0, 0.4, 0.6, 1)                       #alpha
  ...
  Fit_Logistic   = function(df , df_folds = NULL , n_folds = 5, params = NULL){
	...
	cv.lasso <- cv.glmnet(x = xnm, y = df$fraud, alpha = params[[1]], family = "binomial")
      glmnet(x = xnm, y = df$fraud, alpha = params[[1]], family = "binomial", lambda = 		  		cv.lasso$lambda.min)
    })
    
    yhat_train = lapply(c(1:n_folds), function(x) predict(Logistic_models[[x]] ,  	         df[df_folds$train_index[[x]],]))
    yhat_test  = lapply(c(1:n_folds), function(x) predict(Logistic_models[[x]] , df[df_folds$test_index[[x]],]))
...
for (e in start:end){
  ...
  write.csv(Tunning_out , paste0(path, Model , "_" , "Tunning_out.csv") , row.names = F)
  
  summary_table = Tunning_out %>% group_by(P1.alpha) %>%
  ...
  write.csv(summary_table , paste0(path, Model , "_" , "Tunning_summary_out.csv") , row.names = F)
