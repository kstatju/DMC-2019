...
Parallel_function <- function(i, Train, e){
  ...
  cutoff        = seq(0.999,0.94,by = -0.001)# probability cutoff for predicting fraud
...
for (e in start:end){
  ...
  write.csv(Tunning_out , paste0(path, Model , "_",Sampling,"_Tunning_out_set2_std.csv") , row.names = F)
  ...
  write.csv(summary_table , paste0(path, Model , "_", Sampling, "_Tunning_summary_out_set2_std.csv") , row.names = F)
