...
end           = 4320                     # end tune experiment id  RF:162 Nnet:180 Xg:1458 svm:
...
Config_svm   = function(){
  Exp_Params = list()
  Exp_Params[[1]] = c("linear" , "polynomial" , "radial basis" , "sigmoid") #kernel
...
Fit_svm   = function(df , df_folds = NULL , n_folds = 5, params = NULL){
  ...
  svm_models = lapply(c(1:n_folds) , function(x){
    if(Sampling == "down"){
      df_train = downSample(x=df[df_folds$train_index[[x]],!(names(df) %in% c("fraud"))] , 
                            y=as.factor(df[df_folds$train_index[[x]],]$fraud) , 
                            yname = "fraud")
    }
    if(Sampling == "up"){
      df_train = upSample(x=df[df_folds$train_index[[x]],!(names(df) %in% c("fraud"))] , 
                          y=as.factor(df[df_folds$train_index[[x]],]$fraud) , 
                          yname = "fraud")
    }
    if(Sampling == "smote"){
...
for (e in start:end){
  ...
  write.csv(Tunning_out , paste0(path, Model , "_" , Sampling, "_" , "Tunning_out_no_new_dt.csv") , row.names = F)
...
perf_in = read.csv(paste(path, "svm_up_Tunning_out_no_new_dt.csv"), sep = "")
...
write.csv(perf_summary , paste(path, "svm_up_summary_no_new_dt.csv",sep = ""))
